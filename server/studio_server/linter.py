...
def _r_no_animation(files, manifest):
    ...
    for pat in (r"@keyframes\b", r"\btransition(?:-[a-z]+)?\s*:", r"\banimation(?:-[a-z]+)?\s*:\s*(?!false)[a-z0-9]"):
        for line, _ in _lines(pat, js):
            yield Finding("no-animation", ERROR,
...
def _r_no_client_fetch(files, manifest):
    ...
    for line, _ in _lines(r"\bfetch\s*\(|\bXMLHttpRequest\b|new\s+WebSocket", js):
        yield Finding("no-client-fetch", ERROR,
                      "No network in client.js. All fetching lives in server.py.", "client.js", line)
    for line, _ in _lines(r"""<script[^>]+src=|<link[^>]+href=["']https?://""", js):
        yield Finding("no-remote-script", ERROR,
                      "No remote scripts or stylesheets in client.js.", "client.js", line)
...
def _r_no_hardcoded_hex(files, manifest):
    ...
    for line, m in _lines(r"#[0-9a-fA-F]{6}\b|#[0-9a-fA-F]{3}\b", js):
        ...
        yield Finding("no-hardcoded-hex", ERROR,
...
def _r_no_media_queries(files, manifest):
    ...
    for line, _ in _lines(r"@media\b", js):
        yield Finding("no-media-queries", ERROR,
...
def _r_no_custom_fonts(files, manifest):
    ...
    for line, _ in _lines(r"@font-face\b", js):
        yield Finding("no-custom-fonts", ERROR, "No custom fonts; inherit the page font.", "client.js", line)
    ...
    for line, m in _lines(r"font-family\s*:\s*([^;{}]+)", js):
        ...
        if val.startswith(("inherit", "var(")):
            continue
        yield Finding("no-custom-fonts", ERROR,
...
def _r_no_shadow_append(files, manifest):
    ...
    for line, _ in _lines(r"shadow\s*\.\s*append(Child)?\s*\(", js):
        yield Finding("no-shadow-append", ERROR,
...
def _r_export_default(files, manifest):
    ...
    if not re.search(r"export\s+default\s+(async\s+)?(function|\()", js):
        yield Finding("export-default", ERROR,
                      "client.js must `export default function(shadow, ctx) {}`.", "client.js")
    if not re.search(r"shadow\s*\.\s*innerHTML\s*=", js):
        yield Finding("set-innerhtml", WARNING,
                      "Render by setting shadow.innerHTML (idempotent), not by appending.", "client.js")
...
def _r_fragment_branching(files, manifest):
    ...
    if declares and not branches:
        yield Finding("fragment-branching", ERROR,
                      "Declares fragments but client.js never branches on ctx.cell.fragment.", "client.js")
    if not declares:
        yield Finding("fragment-first", WARNING,
                      "No fragments declared. Decompose the widget into independently-placeable "
                      "fragments so it is canvas-native.", "plugin.json")


def _r_phosphor_weight(files, manifest):
    js = files.get("client.js", "")
    for line, _ in _lines(r"\bph-fill\b", js):
        yield Finding("phosphor-weight", WARNING,
                      "Prefer ph-bold over ph-fill (fill blobs on Spectra 6).", "client.js", line)
...
def _r_no_ad_hoc_border(files, manifest):
    ...
    for line, m in _lines(r"\bborder(-(top|bottom|left|right))?\s*:\s*([^;{}]+)", js):
        if "none" in m.group(3).lower():
            continue
        yield Finding("no-ad-hoc-border", WARNING,
...
def _r_no_raise(files, manifest):
    ...
    for line, _ in _lines(r"^\s*raise\b", py, re.MULTILINE):
        yield Finding("no-raise", ERROR,
                      'server.py must not raise; return {"error": "friendly message"}.', "server.py", line)
...
def _r_declare_egress(files, manifest):
    ...
    if not any(isinstance(r, str) and r.startswith("network:") for r in requires):
        yield Finding("declare-egress", ERROR,
...
def _r_data_schema_when_server(files, manifest):
    if "server.py" in files and not manifest.get("data_schema"):
        yield Finding("data-schema", WARNING,
                      "Widget has a server.py but no data_schema. Mine one from real fetch() output "
                      "so its fields are canvas-bindable.", "plugin.json")


def _r_smoke_test(files, manifest):
    smoke = next((p for p in files if p.endswith("test_smoke.py")), None)
    if not smoke:
        yield Finding("smoke-test", WARNING,
                      "Ship a smoke test (tests/test_smoke.py) that renders at every declared size.",
                      "tests/test_smoke.py")


RULES: list[Callable[[dict, dict], Iterable[Finding]]] = [
    _r_no_animation, _r_no_client_fetch, _r_no_hardcoded_hex, _r_no_media_queries,
    _r_no_custom_fonts, _r_no_shadow_append, _r_export_default, _r_fragment_branching,
    _r_phosphor_weight, _r_no_ad_hoc_border, _r_no_raise, _r_declare_egress,
    _r_data_schema_when_server, _r_smoke_test,
]


def lint_widget(files: dict[str, str], manifest: dict[str, Any], *, schema: dict | None = None) -> list[dict]:
