...
const widgetSel = $("widget");
const fragmentSel = $("fragment");
const sizeSel = $("size");
const wInput = $("w");
const hInput = $("h");
const frame = $("frame");
...
function setPill(id, textId, kind, label) {
    const pill = $(id);
...
async function refreshHealth() {
    try {
        ...
        if (h.mode === "disk")
            setPill("mode", "mode-text", "ok", "disk · standalone");
        else if (h.mode === "live")
            setPill("mode", "mode-text", "ok", "live");
        else
            setPill("mode", "mode-text", "bad", "no source");
        ...
        if (h.tesserae !== "ok")
            ...
            setPill("conn", "conn-text", "ok", h.live_data ? "live data + faithful" : "connected");
    }
    catch {
...
async function dataFor(widget) {
    if (state.dataCache.has(widget.key))
        return state.dataCache.get(widget.key);
    try {
...
async function render() {
    if (!state.widget || !state.fragment)
        return;
